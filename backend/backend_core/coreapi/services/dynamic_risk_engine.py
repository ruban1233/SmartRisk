# coreapi/services/dynamic_risk_engine.py
...
class DynamicRiskEngine:
    ...
    Decides TRADE / NO TRADE before strategy selection
    """
    ...
    def expiry_risk(self):
        ...
        if self.capital < 50_000 and self.days_to_expiry <= 1:
            ...
            self.reasons.append(
                "Low capital + near expiry → FORCED NO TRADE"
            )
    ...
    def theta_risk(self):
        ...
        if theta_ratio > 0.03:
            self.total_score += 25
            self.reasons.append("Theta decay > 3% per day")
    ...
    def iv_risk(self):
        if self.iv_level == "high_falling":
            self.total_score += 25
            self.reasons.append("High IV with crash risk")
